...
public class Customer {
	...
	private List<Map<String, Object>> carrito = new ArrayList<>(); //Lista para el carrito
	...
	private void initialize() {
		addNombre(); //Para añadir el nombre del cliente al titulo de la ventana
		...
		menuBar.add(mnOpciones);
			...
			mntmCerrarSesion.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					frameCustomer.dispose(); //Cierrra la ventana Customer
		...
		menuBar.add(mnCarrito);
			...
			mntmPagar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					pagar(); //Despliega ventana de direccion de entrega, datos de pago y tramita el pedido 
				}
			});
			mnCarrito.add(mntmPagar);
			
		principal(); //Permite ver los productos disponibles
...
private void pagar() {
    if(carrito.isEmpty()) { //Si el carrito esta vacio impide avanzar al usuario
        JOptionPane.showMessageDialog(frameCustomer, "Tu carrito está vacío.", "Información", JOptionPane.INFORMATION_MESSAGE);
        return;
    }
    
    direccion(); //Antes de iniciar la tramitacion pide una direccion de entrega
    tarjeta(); //Ademas de una tarjeta valida
...
private void principal() {
    ...
    btnImagenProducto.setBorderPainted(false);
...
private void pedidos() {
    try(Connection conn = DBConnection.getConnection()) {
        ...
        while(rs.next()) { //Repite el bucle hasta el ultimo pedido del usuario
            hayPedidos = true;
...
private void addNombre() {
	try(Connection nombre = DBConnection.getConnection()) {
		...
		if(rsCliente.next()) {
			...
			customerName = "Invitado"; //En caso de haber un error, el usuario pasa a llamarse Invitado
		}
